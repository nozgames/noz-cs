...
public class LocaleGenerator : IIncrementalGenerator
{
        ...
        ImmutableArray<AdditionalText> csvFiles)
    {
        ...
        var defaultLocale = csv.Locales[0];
        ...
        // Locales class with const names
        sb.AppendLine("    public static class Locales");
        sb.AppendLine("    {");
        foreach (var locale in csv.Locales)
        {
            var constName = CapitalizeFirst(locale);
            sb.AppendLine($"        public const string {constName} = \"{locale}\";");
        }
        sb.AppendLine("    }");
        sb.AppendLine();
        ...
        foreach (var locale in csv.Locales)
        {
            ...
            foreach (var entry in csv.Entries)
            {
                var value = entry.Values.ContainsKey(locale)
                    ? entry.Values[locale]
                    : "";
